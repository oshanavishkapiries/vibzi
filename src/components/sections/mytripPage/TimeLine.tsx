  ...
  return (
    <>
      ...
      <div className="space-y-1 text-sm text-muted-foreground">
        ...
        {item?.details?.customFields.arrivalTime &&
          item?.details?.customFields.departureTime && (
            <div className="flex items-center gap-2">
              <Clock className="h-4 w-4" />
              <span>{`${new Date(
                item?.details?.customFields.arrivalTime,
        ...
        {item?.details?.customFields.reservationNumber && (
          <div className="flex items-center gap-2">
            ...
            <span>{item?.details?.customFields.reservationNumber}</span>
          </div>
        )}
      </div>
    </>
...
const SortableItem = ({ item, onEdit, onDelete }: SortableItemProps) => {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={cn("flex gap-4 w-full", isDragging && "opacity-0")}
    >
...
// Main Component
const TimeLine = forwardRef<{ handleSave: () => Promise<void> }, TimeLineProps>(
  ({ data }, ref) => {
